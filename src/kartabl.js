...
import { hashPassword, checkPassword, newPassword, b64, unb64, constantEqual } from './pass.js';
...
/* قالب‌های داخلِ باندل را ورکر هنگامِ بالا آمدن این‌جا ثبت می‌کند
   (src/index.js). عمداً این‌جا import نمی‌شوند: import کردنِ .tpl فقط
   از راهِ باندلر ممکن است و آن‌وقت این فایل با نودِ خالی بالا نمی‌آمد —
   یعنی هر آزمونی که چیزی از همین‌جا می‌خواهد می‌شکست.
   ثبت‌نشده یعنی خالی، و خالی یعنی همان راهِ قبلی از ASSETS. */
let PARTS = new Map(), PAGES = new Map();
export function useBundledTemplates(parts, pages) {
...
export async function withParts(env, req, html) {
  ...
  /* پاره‌ای که داخلِ باندل هست هیچ زیرْدرخواستی خرج نمی‌کند؛ بقیه
     مثلِ قبل از ASSETS خوانده می‌شوند. */
  const got = await Promise.all(names.map(async n => {
    const bundled = PARTS.get(n);
    if (bundled != null) return [n, bundled];
...
export async function renderPanelPage(env, req, panel) {
  ...
  if (html === undefined) {
    let raw = PAGES.get(file);
    if (raw == null) {
      const res = await env.ASSETS.fetch(assetReq(req, file));
      if (!res.ok) return null;
      raw = await res.text();
    }
    html = await withParts(env, req, raw);
...
/* همان‌جایی که بود صادر می‌شود، تا جای دیگری عوض نشود. */
export { hashPassword, checkPassword, newPassword };
