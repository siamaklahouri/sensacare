...
╰──────────────────────────────────────────╯${R}`);
  ...
  const shape = () => {
    ...
    /* دو شکلِ رایج: قدیمی ۴۰ نویسهٔ بی‌پیشوند، تازه cfut_ و ۴۸ نویسه.
       طول به‌تنهایی چیزی را ثابت نمی‌کند، چون هر توکنِ تازه هم همین طول را دارد. */
    info('پیشوندِ cfut_ دارد: ' + (TOKEN.startsWith('cfut_') ? 'بله' : 'نه'));
    /* اثرانگشت: یک‌طرفه است و توکن از رویش درنمی‌آید، ولی می‌گوید رشتهٔ
       رسیده همان قبلی است یا عوض شده — تنها چیزی که طول نمی‌گوید. */
    info('اثرانگشت (۱۲ نویسهٔ اولِ SHA-256): ' +
